...
class Compiler(ast.NodeTransformer):
    ...
    def visit_If(self, node):
        if hasattr(node.test, 'id') and node.test.id == '__debug__':
            return node.orelse
        if not self._main \
                and type(node.test) == ast.Compare \
            ...
            for comparator in node.test.comparators:
                if type(comparator) == ast.Str and comparator.s == '__main__':
                    return node.orelse
        elif hasattr(node.test, 'operand') \
                and type(node.test.op) == ast.Not \
                and type(node.test.operand) == ast.Name \
    ...
    def visit_Expr(self, node):
        if type(node.value) == ast.Call \
                ...
                and node.value.func.attr == 'debug':
            return None
...
def toycompile(data, filename='', path=False,
               obfuscate=False, raw=False, debug=False, main=False):
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '-f', '--fake-file-path', default=False, action='store_true',
        help='Fake file path (number)')
    parser.add_argument(
        '-M', '--delete-main', default=False, action='store_true',
        help='Remove __main__')
    parser.add_argument(
        '-D', '--delete-docstrings', default=False, action='store_true',
        help='Detele __doc__')
    parser.add_argument(
        'files', metavar='<file.py>', nargs='+', help='File(s) to compile')
    args = parser.parse_args(sys.argv[1:])

    def tcompile(fake_filepath, filepath, main=False, docstrings=False):
        ...
        if stat.S_ISDIR(os.stat(f).st_mode):
            for root, _, files in os.walk(f):
                for ff in files:
                    ...
                    tcompile(fname, ff, not args.delete_main,
            ...
            tcompile(fname, f, not args.delete_main,
                     not args.delete_docstrings)
